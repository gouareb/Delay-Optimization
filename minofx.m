function [b,beq,C,Aeq,A,x,fval,exitflag,output]= minofx( Spl,Nr,RrateReq,n,nrequest,npaths,nnode,shortestPaths,totalCosts,RrateAllocated,Xl,linksCapacity,linksRealCapacity,Cnodes,Wkp)
format long
...
for i = 1:nlink
    for y = 1:nBp
        ...
        C(j,:)=1./(linksRealCapacity(i)-n(i,y));
        j=j+1;
    end
end
j2=j;
...
%Mixed-integer linear programming solver initlinprog
Asize=size(A);
AEQsize=size(Aeq);
Fsize=size(C);
